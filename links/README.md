[What is the Default Memory/CPU for a Pod?](https://reuvenharrison.medium.com/kubernetes-resource-limits-defaults-and-limitranges-f1eed8655474)

[Resource Management for Pods and Containers](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#meaning-of-cpu)

[Kubernetes: CPU and Memory Autoscaling](https://medium.com/@myte/kubernetes-autoscaling-abf11314346e)

[Kubernetes Metrics Server](https://github.com/kubernetes-sigs/metrics-server)

[Horizontal Pod Autoscaling](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/)

[Minikube Registries](https://minikube.sigs.k8s.io/docs/handbook/registry/#docker-on-windows)

[wrk - a HTTP benchmarking tool](https://github.com/wg/wrk)

[Designing and implementing API Gateways with Ocelot in .NET Core containers and microservices architectures](https://devblogs.microsoft.com/cesardelatorre/designing-and-implementing-api-gateways-with-ocelot-in-a-microservices-and-container-based-architecture/)

[Ocelot Performance Test](https://github.com/wswind/Ocelot-Performance-Test)

[How to access applications running within minikube](https://minikube.sigs.k8s.io/docs/handbook/accessing/)

[Deploying a multi-container application to Azure Kubernetes Services](https://www.azuredevopslabs.com/labs/vstsextend/kubernetes/)

[Building Ocelot API Gateway Microservices with ASP.NET Core and Docker Container](https://medium.com/aspnetrun/building-ocelot-api-gateway-microservices-with-asp-net-core-and-docker-container-13f96026e86c)

[Sharing a local registry with minikube](https://hasura.io/blog/sharing-a-local-registry-for-minikube-37c7240d0615/)

[Create local docker registry and push images to the registry from minikube](https://medium.com/@heshani.samarasekara/create-local-docker-registry-and-push-images-to-the-registry-from-minikube-f61936278f6c)

[How to setup a local Docker registry with Kubernetes on Windows](https://dev.to/rohansawant/how-to-setup-a-local-docker-registry-with-kubernetes-on-windows-includes-1-hidden-step-that-official-docs-doesn-t-320f)

[PowerShell commands](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/?view=powershell-7.2)

[Using Multi-Node Clusters](https://minikube.sigs.k8s.io/docs/tutorials/multi_node/)

[Environment variables in Compose](https://docs.docker.com/compose/environment-variables/)

[Building ASP.NET Core and EF Core hierarchical multi-tenant apps](https://www.thereformedprogrammer.net/building-asp-net-core-and-ef-core-hierarchical-multi-tenant-apps/)